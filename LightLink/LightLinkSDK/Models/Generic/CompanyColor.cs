...
namespace LightLinkSDK.Models.Generic
{
    ...
    public class CompanyColor
    {
        ...
        /// </summary>
        public int ARGB { get { return A + R + G + B; } }

        /// <summary>
        /// Red Green Blue Values Added together
        /// </summary>
        public int RGB { get { return R + G + B; } }
        ...
        {
            byte[] rgb = ColorHelper.ConvertIntToRGB(ColorHelper.ConvertHexToInt(hex));
            A = 255;
            R = rgb[0];
            G = rgb[1];
            B = rgb[2];
        ...
        /// <summary>
        /// Converts a <see cref="CompanyColor" /> to a <see cref="CorsairColor" />.
        /// </summary>
        /// <param name="color">The <see cref="CompanyColor"/> to convert.</param>
        public static implicit operator CorsairColor(CompanyColor color)
        {            
            return new CorsairColor(color.A, color.R, color.G, color.B);
        }

        /// <summary>
        /// Converts a <see cref="CorsairColor" /> to a <see cref="CompanyColor" />.
        /// </summary>
        /// <param name="color">The <see cref="CorsairColor"/> to convert.</param>
        public static implicit operator CompanyColor(CorsairColor color)
        {
